...
export default function VariantBrePolicy({ onSuccess }) {
    ...
    const existingBrePolicies = useMemo(() => {
        return (
            variantDetail?.VariantBrePolicy?.rulesJson?.rulesJson ||
            variantDetail?.VariantBrePolicy?.rulesJson ||
            []
        );
    }, [variantDetail]);
        ...
        try {
            ...
            if (isEditMode && status === "Draft") {
                ...
                const res = await axios.post(
                    `${process.env.REACT_APP_BACKEND_URL}/associate/variantProduct/createVariantBrePolicy`,
                    payload,
                    { headers: { Authorization: `Bearer ${accessToken}` } }
                );
    ...
    return (
        <Card>
            ...
            <CardHeader title="Variant BRE Policy" />
            <CardContent>
